...
import { X, CheckCircle2, AlertCircle, ArrowRight, Trophy, Sparkles, HelpCircle, Sword } from 'lucide-react';
import confetti from 'canvas-confetti';

export default function QuizRunner({ title, isBossFight = false, questions = [], xpReward = 50, onClose, onComplete }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-xl selection:bg-purple-500 overflow-y-auto">
      <motion.div
        ...
        className="w-full max-w-2xl bg-[#0F172A] border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl shadow-purple-950/60 relative my-6 max-h-[90vh] flex flex-col justify-between"
      >
        {/* Top Glow Line */}
        ...
        <div className="flex items-center justify-between gap-4 border-b border-slate-800 pb-4 mb-4">
          <div className="flex items-center gap-3">
            <div className={`p-2.5 rounded-xl border ${
              ...
              {isBossFight ? <Sword className="w-5 h-5" /> : <HelpCircle className="w-5 h-5" />}
            </div>
            <div>
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-xl bg-slate-950/60 border border-slate-800 text-slate-400 hover:text-white transition-colors cursor-pointer shrink-0"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
          ...
          <div className="py-8 text-center space-y-6 flex flex-col items-center">
            ...
            <div className="space-y-2">
              <span className="px-3 py-1 rounded-full bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 text-xs font-black uppercase tracking-wider">
                {isBossFight ? 'Insígnia Conquistada! 🏆' : 'Avaliação Concluída com Sucesso! 🎉'}
              </span>
              ...
              <p className="text-slate-400 text-xs sm:text-sm max-w-sm mx-auto">
                {isBossFight
                  ? 'Você dominou o conteúdo geral desta Trilha. O aprendizado foi consolidado!'
                  : 'Excelente! Seu conhecimento foi testado e seu progresso registrado com sucesso.'}
              </p>
            </div>

            <div className="flex items-center gap-4 bg-slate-950/80 p-4 rounded-2xl border border-slate-800">
              <div className="flex items-center gap-2 text-purple-300 font-extrabold text-xs sm:text-sm">
                <Sparkles className="w-4 h-4 text-amber-400" />
                <span>+{xpReward} XP Conquistados</span>
              </div>
              <div className="h-4 w-[1px] bg-slate-800" />
              <div className="flex items-center gap-2 text-cyan-400 font-extrabold text-xs sm:text-sm">
