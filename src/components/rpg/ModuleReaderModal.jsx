...
import { X, BookOpen, Sparkles, HelpCircle, ArrowRight } from 'lucide-react';

export default function ModuleReaderModal({ module, onClose, onStartQuiz }) {
      ...
      >
        ...
        <div className="flex items-start justify-between gap-4 border-b border-slate-800 pb-4 mb-6">
          <div className="space-y-1">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-purple-950/60 border border-purple-500/30 text-purple-300 text-xs font-extrabold uppercase tracking-wider">
              ...
              <span>Módulo de Aprendizado Rápido</span>
            </div>
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-xl bg-slate-950/60 border border-slate-800 text-slate-400 hover:text-white transition-colors shrink-0"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="space-y-6 overflow-y-auto pr-2 mb-6 leading-relaxed">
          ...
          <div className="bg-slate-950/60 border border-slate-800/80 rounded-2xl p-5 space-y-3 shadow-inner">
            <div className="flex items-center gap-2 text-purple-400 font-extrabold text-sm uppercase tracking-wider">
              ...
              <span>Contexto Prático & Base Bíblica</span>
            </div>
            <div
          ...
          {/* Insight de Aplicação Section */}
          <div className="bg-gradient-to-r from-purple-950/40 to-cyan-950/30 border border-cyan-500/30 rounded-2xl p-5 space-y-3 shadow-lg">
            <div className="flex items-center gap-2 text-cyan-400 font-extrabold text-sm uppercase tracking-wider">
              <HelpCircle className="w-4 h-4 text-cyan-400" />
              <span>Insight de Aplicação (Púlpito & Redes)</span>
            </div>
            <div
              className="prose prose-invert max-w-none text-slate-200 text-xs sm:text-sm leading-relaxed space-y-3"
        ...
        <div className="pt-4 border-t border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="text-xs font-bold text-slate-400 flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-amber-400" />
            <span>Conclua o Quiz para ganhar +{module.xp} XP e +5 Raios</span>
          </div>
