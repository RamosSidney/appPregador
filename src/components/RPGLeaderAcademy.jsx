...
export default function RPGLeaderAcademy({ userLevel, userXp, onAddXp, onAddCredits }) {
  ...
  const [completedModules, setCompletedModules] = useState(() => {
    try {
      ...
      return saved ? JSON.parse(saved) : ['ga-mod-1'];
    } catch {
      return ['ga-mod-1'];
    }
  });
  ...
  // Handler to open full-screen lesson view
  ...
  // Handler to start quiz from inside Module Content View
  ...
  return (
    <div className="w-full min-h-screen bg-[#0B0E14] text-slate-100 border-none p-0 flex flex-col selection:bg-purple-500 pb-28">
        ...
        <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3 bg-slate-900/80 border border-slate-800/80 p-3 rounded-2xl shadow-lg backdrop-blur-xl">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Buscar por trilha, lição ou tema..."
              className="w-full bg-slate-950/80 border border-slate-800 rounded-xl pl-10 pr-4 py-2 text-xs sm:text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:border-purple-500 transition-colors"
            />
          </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between px-1">
            ...
            <span className="text-xs font-bold text-slate-400">
              {filteredTrails.length} de {ACADEMY_TRAILS.length} Trilhas
            </span>
          </div>
          ...
          {filteredTrails.length === 0 ? (
            <div className="p-8 text-center bg-slate-900/60 border border-slate-800 rounded-2xl space-y-2">
              ...
              <p className="text-slate-300 text-sm font-bold">Nenhuma trilha encontrada.</p>
              <button
      ...
      {/* 100% Full-Screen Lesson Overlay */}
      <AnimatePresence>
        {activeModuleReader && (
          <ModuleContentModal
            key="module-reader"
